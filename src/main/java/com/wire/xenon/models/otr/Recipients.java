...
public class Recipients extends HashMap<Qualified, ClientCipher> {
    ...
    public void add(Qualified userId, String clientId, String cipher) {
        ClientCipher clients = toClients(userId);
        clients.put(clientId, cipher);
    }

    //<UserId, <ClientId, Cipher>>
    public void add(Qualified userId, ClientCipher clients) {
    ...
    public void add(Recipients recipients) {
        Set<Qualified> userIds = recipients.keySet();
        for (Qualified userId : userIds) {
            ClientCipher hashMap = recipients.get(userId);
            add(userId, hashMap);
        }
    }

    private ClientCipher toClients(Qualified userId) {
